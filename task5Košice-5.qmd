...
```{r}
my_real_data <- load_and_inspect("~/Desktop/программирование/iris.csv")
...
# we use the dataset from Task 3
# my_real_data should already contain the iris data
glimpse(my_real_data)
...
```{r}
hist(
  my_real_data$`petal length (cm)`,
  main = "Distribution of Petal Length in Iris Dataset",
...
ggplot(
  data = my_real_data,
  aes(x = `sepal length (cm)`, y = `petal length (cm)`, colour = species)
) +
...
ggplot(
  data = my_real_data,
  aes(x = species, y = `petal length (cm)`, fill = species)
) +
  geom_boxplot() +
  labs(
...
ggplot(
  data = my_real_data,
  aes(x = `petal length (cm)`, fill = species)
) +
...
```{r}
ggsave(
  filename = "iris_petal_length_by_species.png",
  width = 8,
  height = 6
)
